...
public class EmployeeTab {
    ...
    */
    ...
    //TODO: replace with a Binary Search, search an employee by ID
    private void searchEmployee() {
        try {
            String employeeID = employeeIDField.getText().trim();
            DefaultTableModel tableModel = (DefaultTableModel) table.getModel();
            int low = 0;
            int high = tableModel.getRowCount() - 1;

            while (low <= high) {
                int mid = (low + high) / 2;
                String midVal = tableModel.getValueAt(mid, 0).toString();

                if (midVal.compareTo(employeeID) < 0) {
                    low = mid + 1;
                } else if (midVal.compareTo(employeeID) > 0) {
                    high = mid - 1;
                } else {
                    employeeNameField.setText(tableModel.getValueAt(mid, 1).toString());
                    employeeRoleField.setText(tableModel.getValueAt(mid, 2).toString());
                    unpaidSalaryField.setText(tableModel.getValueAt(mid, 4).toString());
                    return;
                }
            }
    ...
    //Create a method to clear forms
