...
public class StudentTab {
    ...
    /* TODO:
        * Fetch and display the student records in the table
        * Store the fetched data in an appropriate data structure*/
    ...
    // TODO: Implement a Binary Search for a student record
    private void searchStudentRecord() {
        try {
            ...
            while (low <= high) {
                int mid = (low + high) / 2;
                String midVal = model.getValueAt(mid, 0).toString();

                if (midVal.compareTo(searchID) < 0) {
                    low = mid + 1;
                } else if (midVal.compareTo(searchID) > 0) {
                    high = mid - 1;
                } else {
                    studentNameField.setText(model.getValueAt(mid, 1).toString());
                    studentGradeField.setText(model.getValueAt(mid, 2).toString());
                    unpaidFeesField.setText(model.getValueAt(mid, 4).toString());
                    return;
                }
            }
    ...
    // Create a method to pay student fees and update the student record
    ...
    // Method to clear the student panel forms
